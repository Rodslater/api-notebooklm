...
async def notify_webhook(job: PodcastJob) -> bool:
    ...
    payload = {
        ...
        "video_format": job.video_format.value if job.generate_video and job.video_engine.value == "notebooklm" else None,
        "video_style": job.video_style.value if job.generate_video and job.video_engine.value == "notebooklm" else None,
